...
export default function MarketCard({ row, header }: { row: any; header: string[] }) {
  return (
    <Card width="100%">
      <Box width="100%" display="flex" flexDirection="column" gap="16px" padding="20px 24px 32px">
        ...
        {header.slice(1, -1).map((headerString, index) => (
          <Box key={index}>
            ...
            <Box mt={8} bgcolor="#F7F7F8" borderRadius="16px" pl={24} display="flex" alignItems="center" height={52}>
              {row[index + 1] ?? null}
            </Box>
          </Box>
        ))}
